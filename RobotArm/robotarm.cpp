...
RobotArm::RobotArm(uint numbody, uint DOF) {
    ...
    // body 0 variable
    Body::ang2mat(M_PI, M_PI_2, -M_PI_2, body[0].Ai, false);
...
void RobotArm::run_inverse_kinematics() {
    ...
    double des_pose[6], conv_pose[6];
    for (uint indx = 0; indx < row; indx++) {
        for(uint i = 0; i < 6; i++){
            des_pose[i] = ref_data[indx*col + i + 8];
        }

        convert_desired(des_pose, body[0].Ai, conv_pose);

        kinematics();
        inverse_kinematics(conv_pose);
...
void RobotArm::kinematics()
{
    ...
    memcpy(body_end->pose, body_end->re, sizeof(double)*3);
    memcpy(body_end->pose + 3, body_end->ori, sizeof(double)*3);
...
void RobotArm::inverse_kinematics(double desired[])
{
    int *indx = new int[6];
    ...
    for(int i = 0; i < 1/alpha; i++){
        for(int j = 0; j < 6; j++){
            err[j] = desired[j] - body[num_body].pose[j];
        }
...
void RobotArm::convert_desired(double desired_origin[], double rot_mat[], double desired_conv[])
{
    double des_pos[3], des_rot[3];
    memcpy(des_pos, desired_origin, sizeof(double)*3);
    memcpy(des_rot, desired_origin + 3, sizeof(double)*3);

    double des_mat[9], conv_mat[9], conv_rot[3], conv_pos[3];
    rpy2mat(des_rot[2], des_rot[1], des_rot[0], des_mat);
    mat(rot_mat, des_mat, 3,3, 3,3, conv_mat);
    mat2rpy(conv_mat, conv_rot);
    mat(rot_mat, des_pos, 3, 3, 3, conv_pos);

    memcpy(desired_conv, conv_pos, sizeof(double)*3);
    memcpy(desired_conv + 3,  conv_rot, sizeof(double)*3);
}
...
void RobotArm::jacobian()
{
    ...
    for(uint indx = 1; indx <= num_body; indx++){
        ...
        mat(body[indx].Ai_Cij, body[indx-1].u_vec, 3, 3, 3, body[indx].zi);
        tilde(body[indx].zi, body[indx].zit);
        mat(body[indx].zit, body[indx].oi, 3, 3, 3, body[indx].Jvi);

        for(uint i = 0; i < 3; i++){
            Jv[i*6 + indx - 1] = body[indx].Jvi[i];
            Jw[i*6 + indx - 1] = body[indx].zi[i];
        }
    }
...
void RobotArm::save_data() {
    ...
    double conv_pos[3], conv_rot[3], conv_mat[9];
    for(uint i = 0; i < 3; i++){
        conv_pos[i] = 0;
        for(uint j = 0; j < 3; j++){
            conv_pos[i] += body[0].Ai[j*3 + i]*body[num_body].re[j];
        }
    }
    ...
    for(uint i = 0; i < 3; i++){
        for(uint j = 0; j < 3; j++){
            conv_mat[i*3 + j] = 0;
            for(uint k = 0; k < 3; k++){
                conv_mat[i*3 + j] += body[0].Ai[k*3 + i]*body[num_body].Ae[k*3 + j];
            }
        }
    }
    mat2rpy(conv_mat, conv_rot);

    fprintf(fp, "%.7f\t%.7f\t%.7f\t", conv_pos[0], conv_pos[1], conv_pos[2]);
    fprintf(fp, "%.7f\t%.7f\t%.7f\t", conv_rot[0], conv_rot[1], conv_rot[2]);
